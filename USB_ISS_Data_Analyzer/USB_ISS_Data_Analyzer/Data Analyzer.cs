...
{
    /**
     *  This file contains the algorithm for measuring the number of steps taken based on a pedometer's accelerometer.
     **/

    class Data_Analyzer
    {
        /**
         *  This section contains all of our essentials to run and construct the algorithm 
         **/
        ...
        int m_x_offSet, m_y_offSet, m_z_offSet;
        ...
        static private CMPS10_INTERFACE m_compass_interface;

        //Constructor - Initialiser as well
        public Data_Analyzer()
        {
            m_compass_interface = new CMPS10_INTERFACE();
            m_devStatus = m_compass_interface.Open();

            m_x_offSet = m_y_offSet = m_z_offSet = 0;
            if (!m_devStatus)
            {
                MessageBox.Show("Cannot initialize analyzer - No device context!");
            }
        ...
        public void CalculateSteps(float dt)
        {
            ...
            if (isRunning)
            {
                ...
                if (cycle_count > 8 && total_samples > 8)
                {
                    ...
                    //to keep the averages updated and manage resources(i.e. instead of using a list that grows into O(n^x))
        ...
        private bool IsStep(float avg, float oldavg)
        {
            ...
            //If we are here, it means the person has moved up
            ...
            //If we are here, it means the person has moved down
        ...
        {
            int [] data = m_compass_interface.GetAcceleration();
            int [] data_convert = new int[3];
            
            data_convert[0] = (data[2]);
            data_convert[1] = (data[5]);
            data_convert[2] = (data[8]);

            return data_convert;
        }

        //Read angle data and return them as values
        private void Get_Angles()
        {
            m_angle_data[0] = Convert.ToInt16(m_compass_interface.GetAngle());
            m_angle_data[1] = Convert.ToInt16(m_compass_interface.GetPitch());
            m_angle_data[2] = Convert.ToInt16(m_compass_interface.GetRoll());
        }
